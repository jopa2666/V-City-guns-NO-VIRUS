...
local npc_youseeme2_acquire_distance =
	CreateConVar("npc_youseeme2_acquire_distance", 100500, FCVAR_NONE,
	"The maximum distance at which youseeme2 will chase a target.")

local npc_youseeme2_attack_distance =
	CreateConVar("npc_youseeme2_attack_distance", 80, FCVAR_NONE,
	"The reach of youseeme2's attack.")

local npc_youseeme2_attack_interval =
	CreateConVar("npc_youseeme2_attack_interval", 0.1, FCVAR_NONE,
	"The delay between youseeme2's attacks.")

local npc_youseeme2_attack_force =
	CreateConVar("npc_youseeme2_attack_force", 800, FCVAR_NONE,
	"The physical force of youseeme2's attack. Higher values throw things \z
	farther.")

local npc_youseeme2_smash_props =
	CreateConVar("npc_youseeme2_smash_props", 1, FCVAR_NONE,
	"If set to 1, youseeme2 will punch through any props placed in their way.")

local npc_youseeme2_allow_jump =
	CreateConVar("npc_youseeme2_allow_jump", 0, FCVAR_NONE,
	"If set to 1, youseeme2 will be able to jump.")

local npc_youseeme2_chase_repath_interval =
	CreateConVar("npc_youseeme2_chase_repath_interval", 0.1, FCVAR_NONE,
	"The path to and position of youseeme2's target will be redetermined every \z
	X seconds.")

local npc_youseeme2_expensive_scan_interval =
	CreateConVar("npc_youseeme2_expensive_scan_interval", 1, FCVAR_NONE,
...
util.AddNetworkString("youseeme2_nag")
util.AddNetworkString("youseeme2_navgen")
...
local VECTOR_youseeme2_HEIGHT = Vector(0, 0, 96)
...
local function isValidTarget(ent)
	...
	-- Ignore dead NPCs, other youseeme2s, and dummy NPCs.
	local class = ent:GetClass()
	return (ent:IsNPC()
		and ent:Health() > 0
		and class ~= "npc_youseeme2"
		and not class:find("bullseye"))
end

hook.Add("PlayerSpawnedNPC", "youseeme2MissingNavmeshNag", function(ply, ent)
	if not IsValid(ent) then return end
	if ent:GetClass() ~= "npc_youseeme2" then return end
	if navmesh.GetNavAreaCount() > 0 then return end

	-- Try to explain why youseeme2 isn't working.
	net.Start("youseeme2_nag")
	net.Send(ply)
end)
...
local function navEndGenerate()
	...
	if not navmesh.IsGenerating() then
		print("npc_youseeme2: Navmesh generation completed in " .. timeElapsedStr)
	else
		print("npc_youseeme2: Navmesh generation aborted after " .. timeElapsedStr)
	end
...
local function navGenerate()
	...
	if next(seeds, nil) == nil then
		print("npc_youseeme2: Couldn't find any places to seed nav_generate")
		return false
	end
	...
	for seed in pairs(seeds) do
		...
		if not tr.StartSolid and tr.Hit then
			print(string.format("npc_youseeme2: Adding seed %s at %s", seed, pos))
			navmesh.AddWalkableSeed(tr.HitPos, tr.HitNormal)
		else
			print(string.format("npc_youseeme2: Couldn't add seed %s at %s", seed,
				pos))
		end
	end

	-- The least we can do is ensure they don't have to listen to this noise.
	for _, youseeme2 in pairs(ents.FindByClass("npc_youseeme2")) do
		youseeme2:Remove()
	end
	...
	if navmesh.IsGenerating() then
		...
		hook.Add("ShutDown", "youseeme2NavGen", navEndGenerate)
	else
		print("npc_youseeme2: nav_generate failed to initialize")
...
concommand.Add("npc_youseeme2_learn", function(ply, cmd, args)
	if navmesh.IsGenerating() then
		return
	end
	...
	if game.SinglePlayer() then
		print("npc_youseeme2: Beginning nav_generate requested by " .. ply:Name())

		-- Disable expensive computations in single-player. youseeme2 doesn't use
	...
	elseif isConsole then
		print("npc_youseeme2: Beginning nav_generate requested by server console")
	else
		return
	end
...
function ENT:GetNearestTarget()
	local maxAcquireDist = npc_youseeme2_acquire_distance:GetInt()
...
function ENT:AttackNearbyTargets(radius)
	local attackForce = npc_youseeme2_attack_force:GetInt()
	...
	for _, ent in pairs(nearEntities) do
		if isValidTarget(ent) then
			...
			-- Give the player a good whack. youseeme2 means business.
		...
		elseif ent:GetMoveType() == MOVETYPE_VPHYSICS then
			if not npc_youseeme2_smash_props:GetBool() then continue end
...
--[[function ENT:AttemptJumpAtTarget()
	...
	local maxAttackDistance = npc_youseeme2_attack_distance:GetInt()
...
function ENT:BehaveUpdate()
    ...
    if currentTime - self.LastTargetSearch > npc_youseeme2_expensive_scan_interval:GetFloat() then
    ...
    if IsValid(self.CurrentTarget) then
        ...
        local repathInterval = npc_youseeme2_chase_repath_interval:GetFloat()
    ...
    if IsValid(self.CurrentTarget) then
        if currentTime - self.LastAttack > npc_youseeme2_attack_interval:GetFloat() then
            local attackDistance = npc_youseeme2_attack_distance:GetInt()
...
local MAT_youseeme2 = Material("homigrad/scp/scared/youseemee2.png")
killicon.Add("npc_youseeme2", "npc_youseeme2/killicon", color_white)
language.Add("npc_youseeme2", "youseeme2 ")
...
function ENT:DrawTranslucent()
	if math.random(1,5) != 1 then
		...
		render.SetMaterial(MAT_youseeme2)
	else
		render.SetMaterial(Material("null.vmt"))
	end

	-- Get the normal vector from youseeme2 to the player's eyes, and then compute
	...
	-- youseeme2 should only look 1/3 of the way up to the player so that they
